...
/**
 * HTTP client for forwarding SMS messages to external APIs.
    ...
    suspend fun forwardSms(smsMessage: SmsMessage, rule: Rule): ForwardingResult {
        ...
        return executeWithRetry(
            ...
            when (rule.method.uppercase()) {
                "GET" -> {
                    val queryParams = payload.mapValues { it.value.toString() }
                    apiService.getFromEndpoint(rule.endpoint, headers, queryParams)
                }
                "POST" -> {
                    apiService.postToEndpoint(rule.endpoint, headers, payload)
                }
                "PUT" -> {
                    apiService.putToEndpoint(rule.endpoint, headers, payload)
                }
                "PATCH" -> {
                    apiService.patchToEndpoint(rule.endpoint, headers, payload)
                }
                else -> {
...
/**
 * Result of an SMS forwarding attempt.
 */
